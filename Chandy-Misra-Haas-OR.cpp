...
int main() {
	...
	int pid_probe;
	char option;
    cout<<" \n Welcome to Chandy-Misra-Haas OR Model Deadlock Detection Algorithm \n";
    cout<<" Press Enter for Contine \n";
...
void detectDeadlock(vector<std::vector<int>> &graph, int init, int dest)
{
	...
	for (int col = 0; col < end; col++)
	{
		if (graph[dest][col] == 1)
		{
			...
			{
                count +=1 ;
                cout << " S" << (dest + 1) << " --> S" << (col + 1) <<"     (" << (init + 1) << "," << (dest + 1) << "," << (col + 1) << ")"<< endl;//<< " --------> DEADLOCK DETECTED HERE" << endl;
                break;
			}
